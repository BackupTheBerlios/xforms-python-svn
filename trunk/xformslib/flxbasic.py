...
def fl_is_global_clipped():
    """fl_is_global_clipped() -> yesno

    Determines if global clipping is switched on or not.

    Returns
    -------
        yesno : int
            flag if global clipping is switched on or off

    Examples
    --------
        >>> clipd = fl_is_global_clipped()
    ...
    """
    _fl_is_global_clipped = library.cfuncproto(
        library.load_so_libforms(), "fl_is_global_clipped",
        cty.c_int, [],
        """int fl_is_global_clipped( void )""")
    library.check_if_flinitialized()
    retval = _fl_is_global_clipped()
    return retval


def fl_is_clipped(includeglobal):
    ...
    ----------
        includeglobal : int
            flag (if set) for including global clipping (used e.g.
            when redrawing on an expose event), otherwise only
            clipping set via a call of fl_set_clipping() is
            reported.

    Returns
    -------
        yesno : int
            flag if clipping is switched on or off

    Examples
    --------
        >>> clipd = fl_is_clipped(1)???
    ...
    """
    _fl_is_clipped = library.cfuncproto(
        library.load_so_libforms(), "fl_is_clipped",
        None, [cty.c_int],
        """int fl_is_clipped( int include_global )""")
    library.check_if_flinitialized()
    i_includeglobal = library.convert_to_intc(includeglobal)
    library.keep_elem_refs(includegloabl, i_includegloabl)
    retval = _fl_is_clipped(i_includeglobal)
    return retval


def fl_is_text_clipped(includeglobal):
    """fl_is_text_clipped(includeglobal) -> yesno

    Determines if "text" clipping is switched on.
    ...
    """
    _fl_is_text_clipped = library.cfuncproto(
        library.load_so_libforms(), "fl_is_text_clipped",
        None, [cty.c_int],
        """int fl_is_text_clipped( int include_global )""")
    library.check_if_flinitialized()
    i_includeglobal = library.convert_to_intc(includeglobal)
    library.keep_elem_refs(includegloabl, i_includegloabl)
    retval = _fl_is_text_clipped(i_includeglobal)
    return retval
...
def fl_get_global_clipping():
    ...
    -------
        yesno : int
            flag if global clipping is switched on or not
        xpos : int
            horizontal position of the clipping rectangle
        ypos : int
            vertical position of the clipping rectangle
        width : int
    ...
    """
    _fl_get_global_clipping = library.cfuncproto(
        library.load_so_libforms(), "fl_get_global_clipping",\
        cty.c_int, [cty.POINTER(xfdata.FL_Coord),
        cty.POINTER(xfdata.FL_Coord), cty.POINTER(xfdata.FL_Coord),
        cty.POINTER(xfdata.FL_Coord)],\
    ...
    i_width, ptr_width = library.make_FL_Coord_and_pointer()
    i_height, ptr_height = library.make_FL_Coord_and_pointer()
    library.keep_elem_refs(i_xpos, i_ypos, i_width, i_height, ptr_xpos, \
            ptr_ypos, ptr_width, ptr_height)
    retval = _fl_get_global_clipping(ptr_xpos, ptr_ypos, ptr_width, \
            ptr_height)
    return retval, i_xpos.value, i_ypos.value, i_width.value, i_height.value


def fl_get_clipping(includeglobal):
    """fl_get_clipping(includeglobal) -> yesno, xpos, ypos, width, height

    Determines if normal clipping (e.g. "non-text") is on and the region
    clipped to 

    Parameters
    ----------
        includeglobal : int
            flag; if set also clipping due to global clipping is taken into
            account (and the reported clipping region is the one resulting
            from intersection of the normal clipping region and the one for
            global clipping).

    Returns
    -------
        yesno : int
            flag if there is clipping going on (which may, if includeglobal
            is set, also be due to global clipping)
        xpos : int
            horizontal position of the clipping rectangle
        ypos : int
            vertical position of the clipping rectangle
    ...
    """
    _fl_get_clipping = library.cfuncproto(
        library.load_so_libforms(), "fl_get_clipping",\
        cty.c_int, [cty.c_int, cty.POINTER(xfdata.FL_Coord),
        cty.POINTER(xfdata.FL_Coord), cty.POINTER(xfdata.FL_Coord),
        cty.POINTER(xfdata.FL_Coord)],\
        """int fl_get_clipping( int includegloabl, FL_COORD * x,
           FL_COORD * y, FL_COORD * w, FL_COORD * h )""")
    library.check_if_flinitialized()
    i_includegloabl = library.convert_to_intc(includeglobal)
    i_xpos, ptr_xpos = library.make_FL_Coord_and_pointer()
    i_ypos, ptr_ypos = library.make_FL_Coord_and_pointer()
    i_width, ptr_width = library.make_FL_Coord_and_pointer()
    i_height, ptr_height = library.make_FL_Coord_and_pointer()
    library.keep_elem_refs(includeglobal, i_includeglobal, i_xpos, i_ypos, \
            i_width, i_height, ptr_xpos, ptr_ypos, ptr_width, ptr_height)
    retval = _fl_get_clipping(i_includeglobal, ptr_xpos, ptr_ypos, \
...
def fl_get_text_clipping(includeglobal):
    ...
    ----------
        includeglobal : int
            flag; if set also clipping due to global clipping is taken into
            account (and the reported clipping region is the one resulting
            from intersection of the text clipping region and the one for
            global clipping).
    ...
    """
    _fl_get_text_clipping = library.cfuncproto(
        library.load_so_libforms(), "fl_get_text_clipping",\
        cty.c_int, [cty.c_int, cty.POINTER(xfdata.FL_Coord),
        cty.POINTER(xfdata.FL_Coord), cty.POINTER(xfdata.FL_Coord),
        cty.POINTER(xfdata.FL_Coord)],\
        """int fl_get_text_clipping( int includegloabl, FL_COORD * x,
           FL_COORD * y, FL_COORD * w, FL_COORD * h )""")
    library.check_if_flinitialized()
    i_includegloabl = library.convert_to_intc(includeglobal)
    i_xpos, ptr_xpos = library.make_FL_Coord_and_pointer()
    ...
    library.keep_elem_refs(includeglobal, i_includeglobal, i_xpos, i_ypos, \
            i_width, i_height, ptr_xpos, ptr_ypos, ptr_width, ptr_height)
    retval = _fl_get_text_clipping(i_includeglobal, ptr_xpos, ptr_ypos, \
            ptr_width, ptr_height)
    return retval, i_xpos.value, i_ypos.value, i_width.value, i_height.value
