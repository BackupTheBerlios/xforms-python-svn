...
def fl_add_io_callback(fd, mask, py_IoCallback, vdata):
    ...
    libr.checkfatal_allowed_value_in_list(mask, xfdata.ASYNCIO_list)
...
def fl_input_end_return_handling(endtype):
    ...
    libr.check_if_initialized()
    libr.checkfatal_allowed_value_in_list(endtype, \
...
def fl_bgn_form(formtype, w, h):
    ...
    libr.check_if_initialized()
    libr.checkfatal_allowed_value_in_list(formtype, xfdata.BOXTYPE_list)
...
def fl_set_object_resize(pFlObject, what):
    ...
    libr.check_if_initialized()
    libr.verify_flobjectptr_type(pFlObject)
    libr.checkfatal_allowed_value_in_list(what, xfdata.RESIZE_list)
...
def fl_set_object_gravity(pFlObject, nw, se):
    ...
    libr.check_if_initialized()
    libr.verify_flobjectptr_type(pFlObject)
    libr.checkfatal_allowed_value_in_list(nw, xfdata.GRAVITY_list)
    libr.checkfatal_allowed_value_in_list(se, xfdata.GRAVITY_list)
...
def fl_notify_object(pFlObject, cause):
    ...
    libr.check_if_initialized()
    libr.verify_flobjectptr_type(pFlObject)
    libr.checkfatal_allowed_value_in_list(cause, xfdata.EVENTS_list)
...
def fl_drw_text_beside(align, x, y, w, h, colr, style, size, txtstr):
    ...
    libr.check_if_initialized()
    libr.checkfatal_allowed_value_in_list(align, xfdata.ALIGN_list)
    libr.checknonfatal_allowed_value_in_list(align, xfdata.COLOR_list)
    libr.checkfatal_allowed_value_in_list(style, xfdata.TEXTSTYLE_list)
    libr.checknonfatal_allowed_value_in_list(size, xfdata.FONTSIZE_list)
    ialign = libr.convert_to_int(align)
    ix = libr.convert_to_FL_Coord(x)
...
def fl_drw_text_cursor(align, x, y, w, h, colr, style, size, txtstr, curscolr,
    ...
    libr.checkfatal_allowed_value_in_list(style, xfdata.TEXTSTYLE_list)
    libr.checknonfatal_allowed_value_in_list(size, xfdata.FONTSIZE_list)
    libr.checknonfatal_allowed_value_in_list(curscolr, xfdata.COLOR_list)
    ialign = libr.convert_to_int(align)
    ix = libr.convert_to_FL_Coord(x)
    iy = libr.convert_to_FL_Coord(y)
    iw = libr.convert_to_FL_Coord(w)
...
def fl_make_object(objclass, objtype, x, y, w, h, label, py_HandlePtr):
    ...
    _fl_make_object = libr.cfuncproto(
        ...
        """FL_OBJECT * fl_make_object(int objclass, int type, FL_Coord x,
           ...
           FL_HANDLEPTR handle)""")
    libr.check_if_initialized()
    libr.checkfatal_allowed_value_in_list(objclass, xfdata.OBJCLASS_list)
    iobjclass = libr.convert_to_int(objclass)
...
def fl_set_coordunit(unit):
    ...
    libr.check_if_initialized()
    libr.checkfatal_allowed_value_in_list(unit, xfdata.COORDUNIT_list)
...
def fl_set_scrollbar_type(sbtype):
    ...
    libr.checkfatal_allowed_value_in_list(sbtype, xfdata.SCROLLTYPE_list)
