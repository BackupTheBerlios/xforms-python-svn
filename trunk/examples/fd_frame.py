...
class Fd_frame(object):
    def __init__(self, lsysargv, sysargv):
        xfl.fl_initialize(lsysargv, sysargv, 'Fd_frame', None, 0)
    ...
    def create_forms(self):
        ...
        self.pframe = xfl.fl_add_frame(xfl.FL_ENGRAVED_FRAME, 30, 30, 260, 90, 'MyFrame')
        xfl.fl_set_object_boxtype(self.pframe, xfl.FL_FLAT_BOX)
        xfl.fl_set_object_color(self.pframe, xfl.FL_BLACK, xfl.FL_COL1)
        xfl.fl_set_object_lalign(self.pframe, xfl.FL_ALIGN_CENTER)
        xfl.fl_set_object_lstyle(self.pframe, xfl.FL_NORMAL_STYLE)
        xfl.fl_set_object_lsize(self.pframe, xfl.FL_DEFAULT_SIZE)
        xfl.fl_set_object_lcol(self.pframe, xfl.FL_BLACK)
        xfl.fl_set_object_resize(self.pframe, xfl.FL_RESIZE_ALL)
        xfl.fl_set_object_gravity(self.pframe, xfl.FL_NoGravity, xfl.FL_NoGravity)
        xfl.fl_set_object_shortcut(self.pframe, 'F', 1)
        xfl.fl_set_object_callback(self.pframe, self.framecb, 1)

        xfl.fl_end_form()

    def framecb(self, pobj, data):
        pass


if __name__ == '__main__':
    print("***** fd_frame.py *****")
    ApplDemo = Fd_frame(len(sys.argv), sys.argv)
