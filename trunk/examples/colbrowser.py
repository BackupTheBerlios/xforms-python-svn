...
#rgbfile_alt = "/usr/share/X11/rgb.txt"
...
class FlColbrowser(object):
    def __init__(self, lsysargv, sysargv):
        ...
        xfl.fl_set_form_minsize(self.pcl, self.pcl.contents.w , self.pcl.contents.h)
        xfl.fl_set_form_maxsize(self.pcl, 2 * self.pcl.contents.w , 2 * self.pcl.contents.h)
        xfl.fl_show_form(self.pcl, xfl.FL_PLACE_FREE, xfl.FL_TRANSIENT, "RGB Browser")
    ...
    def set_entry(self, i):
        #RGBdb *db = rgbdb + i;
        #print("i", i, "rgbdb[i].r", rgbdb[i].r, "rgbdb[i].g",  rgbdb[i].g, "rgbdb[i].b",  rgbdb[i].b)

        xfl.fl_freeze_form(self.pcl)
        xfl.fl_mapcolor(xfl.FL_FREE_COL4 + i, self.rgbdb[i]['r'], self.rgbdb[i]['g'], self.rgbdb[i]['b'])
        xfl.fl_mapcolor(xfl.FL_FREE_COL4, self.rgbdb[i]['r'], self.rgbdb[i]['g'], self.rgbdb[i]['b'])
    ...
    def read_entry(self, dataline):
        ...
        dataline1 = dataline1.replace("   ", " ")
        dataline1 = dataline1.replace("  ", " ")
        listline = dataline1.split()
        r = (listline[0])
        g = (listline[1])
        b = (listline[2])
        namel = listline[3:]
    ...
    def create_form_cl(self):
        ...
        self.pdbobj = xfl.fl_add_button(xfl.FL_NORMAL_BUTTON, 40, 50, 250, 25, "")
        ...
        xfl.fl_set_object_color(self.prescol, xfl.FL_FREE_COL4, xfl.FL_FREE_COL4)
        xfl.fl_set_object_boxtype(self.prescol, xfl.FL_BORDER_BOX)
        xfl.fl_set_object_resize(self.prescol, xfl.FL_RESIZE_NONE)
        xfl.fl_set_object_gravity(self.prescol, xfl.FL_NorthEast, xfl.FL_East)

        self.prs = xfl.fl_add_valslider(xfl.FL_VERT_FILL_SLIDER, 225, 130, 30, 200, "")
        ...
        xfl.fl_set_object_gravity(self.prs, xfl.FL_NorthEast, xfl.FL_SouthEast)
        xfl.fl_set_object_return(self.prs, xfl.FL_RETURN_CHANGED)

        self.pgs = xfl.fl_add_valslider(xfl.FL_VERT_FILL_SLIDER, 255, 130, 30, 200, "")
        ...
        xfl.fl_set_object_gravity(self.pgs, xfl.FL_NorthEast, xfl.FL_SouthEast)
        xfl.fl_set_object_return(self.pgs, xfl.FL_RETURN_CHANGED)

        self.pbs = xfl.fl_add_valslider(xfl.FL_VERT_FILL_SLIDER, 285, 130, 30, 200, "")
        ...
        xfl.fl_set_object_gravity(self.pbs, xfl.FL_NorthEast, xfl.FL_SouthEast)
        xfl.fl_set_object_return(self.pbs, xfl.FL_RETURN_CHANGED)

        self.pcolbr = xfl.fl_add_browser(xfl.FL_HOLD_BROWSER, 10, 90, 205, 240, "")
        xfl.fl_set_browser_fontstyle(self.pcolbr, xfl.FL_FIXED_STYLE)
        xfl.fl_set_object_callback(self.pcolbr, self.br_cb, 0)
        xfl.fl_set_object_gravity(self.pcolbr, xfl.FL_NorthWest, xfl.FL_SouthEast)

        pobj = xfl.fl_add_button(xfl.FL_NORMAL_BUTTON, 135, 345, 80, 30, "Done")
