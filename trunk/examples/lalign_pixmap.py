...
def main(lsysargv, sysargv):
    ...
    align = xf.fl_get_object_lalign(fd_form0.box) | xfc.FL_ALIGN_INSIDE
    if align != xf.fl_get_object_lalign(fd_form0.box):
...
def inside_cb(pobj, data):
    ...
    if xf.fl_get_button(pobj):
        newalign = xf.fl_get_object_lalign(fd_form0.box)
        newalign |= xfc.FL_ALIGN_INSIDE
        xf.fl_set_object_lalign(fd_form0.box, newalign)
    ...
    if TEST_PIXMAP_ALIGN:
        xf.fl_set_pixmap_align(fd_form0.box, xf.fl_get_object_lalign(fd_form0.box), 3, 3)
    else:
